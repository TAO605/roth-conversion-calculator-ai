...
export default async function GlossaryTermPage({ params }: GlossaryTermPageProps) {
  ...
  return (
    <main className="mx-auto grid max-w-3xl gap-7 px-4 py-10">
      ...
      <article className="rounded-lg border border-neutral-200 bg-white p-5 text-base leading-8 text-neutral-700 dark:border-white/10 dark:bg-neutral-950 dark:text-neutral-200">
        {term.definition}
      </article>
      <section className="rounded-lg border border-neutral-200 bg-white p-5 dark:border-white/10 dark:bg-neutral-950">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {relatedTerms.map((relatedTerm) => (
            <Link
              className="rounded-md bg-blue-500/10 px-3 py-2 text-sm font-semibold text-systemBlue"
