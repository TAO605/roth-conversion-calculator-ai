...
export const metadata = {
  ...
  description:
    "Educational Roth conversion planning checklist for preparing calculator inputs, account records, assumptions, model-limit review, and CPA handoff before using a Roth conversion calculator.",
  alternates: { canonical: "/roth-conversion-planning-checklist" },
};

export default function RothConversionPlanningChecklistPage() {
    ...
    <main className="mx-auto grid max-w-6xl gap-8 px-4 py-10">
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(
            breadcrumbJsonLd([
              ...
              { name: "Roth Conversion Planning Checklist", path: "/roth-conversion-planning-checklist" },
            ]),
          ),
        }}
      ...
      <header className="rounded-[22px] bg-white/75 p-6 shadow-material backdrop-blur-xl dark:bg-white/10">
        ...
        <p className="mt-4 max-w-3xl text-lg leading-8 text-neutral-600 dark:text-neutral-300">
          Use this educational checklist to prepare calculator inputs, account records, assumptions, and review questions
          before modeling a Roth conversion. It helps structure a scenario; it does not recommend a conversion.
        </p>
        <div className="mt-5 flex flex-wrap gap-3 text-sm">
          <span className="rounded-full bg-blue-500/10 px-3 py-1 font-semibold text-systemBlue">
            {summary.totalGroups} planning areas
          </span>
                ...
                >
                  ...
                  {entry.calculatorInput ? (
                    <span className="w-fit rounded-full bg-blue-500/10 px-3 py-1 text-xs font-semibold text-systemBlue">
                      Calculator input: {entry.calculatorInput}
                    </span>
                  ) : null}
                </div>
              ))}
