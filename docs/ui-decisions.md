...
## 2026-05-30 - GSC Queries Become Review-Gated Content Actions

The SEO monitoring page now includes a query opportunity matrix so Search Console data can drive content work without loosening YMYL boundaries.

Decision:

- Cluster queries by intent rather than publishing one page per keyword variant.
- Map each cluster to a target surface: homepage, results, tax guide, state page, filing-status page, payment-method page, or CPA handoff page.
- Require professional review before new bracket formulas, tax-year data, state-specific rules, or external program estimates are added.
- Keep recommendation, optimal-action, and absolute-accuracy phrasing blocked by tests.

## 2026-05-30 - Search Console Exceptions Need Their Own Queue

The SEO monitoring page now separates Google workflow exceptions from site regressions.

Decision:

- Track domain-property DNS verification, sitemap submission, and URL Inspection request-indexing outcomes as explicit exceptions.
- Do not change site code just because Search Console request indexing returns a transient error after live testing passes.
- Keep URL-prefix property operations active while domain-wide DNS verification is unresolved.
- Record retry windows so agents do not repeatedly click request-indexing in a short period.

## 2026-05-30 - GSC Retry Evidence Is Command-Driven

Search Console retry readiness now has a public URL evidence command.

Decision:

- Add `npm run seo:gsc-evidence` for priority URL status, canonical, sitemap inclusion, and noindex checks.
