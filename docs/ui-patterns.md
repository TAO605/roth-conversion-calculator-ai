...
- Browser security headers should be checked as live production evidence, not only static `next.config.ts` intent; retain CSP, HSTS, nosniff, referrer, permissions, frame, base URI, form-action, and no-powered-by status in `security-headers-evidence-result.json`.
...
- Retain crawl-discovery endpoints as production evidence instead of relying only on smoke status: the artifact should prove robots.txt lists sitemap/feed/llms, sitemap URLs stay on the canonical host, RSS retains blog item coverage, and llms.txt keeps core site discovery plus educational boundary language.
- Retain internal-link health as production evidence instead of relying only on sitemap presence: the artifact should prove every sitemap URL returns HTTP 200 on the canonical host without noindex, and `/site-index` keeps enough core internal links to support crawl depth review.
- Retain HTML quality as production evidence instead of relying only on source tests: the artifact should prove production pages keep `lang=en`, one H1, title, meta description, canonical tags, image alt text, named buttons, and labeled form controls.
- Retain professional review packet readiness as production evidence instead of relying only on page copy: the artifact should prove `/professional-review-packet` is live, discoverable in sitemap and `llms.txt`, aligned with `/api/health`, and still explicit that tax professional review is pending.
- Retain Search Console verification readiness as production evidence instead of relying only on screenshots: the artifact should prove the public DNS TXT token, SPF record, homepage Google verification meta tag, and canonical host are visible, while explicitly not asserting private Google Search Console UI ownership state.
- Treat GSC URL Inspection results as external manual evidence, not site-side truth: use a structured record template with exact observed status, Google-selected canonical, retry outcome, screenshots, and linked production SEO evidence before changing code or escalating indexing issues.
- Use AI-assisted GSC indexing record drafts to prefill only public artifact evidence such as run id, commit SHA, Search Console verification readiness, internal-link health, and HTML quality. Draft records must keep private URL Inspection status, Google-selected canonical, request-indexing response, and screenshot path as placeholders until a reviewer copies the real GSC UI result.
- Add a readiness report before archive or retry notes so missing private GSC fields are explicit: reviewer identity, indexing state, live-test state, Google-selected canonical, exact request-indexing message, screenshot path, and notes.
- Generate handoff summaries only from completed recorded GSC indexing records. Draft summaries should remain marked not ready and list blocking fields instead of turning public site evidence into a private indexing claim.
- Keep a machine-readable GSC indexing records manifest for archived private Search Console evidence. The manifest should inventory recorded JSON files, screenshot file existence, byte sizes, and sha256 hashes so screenshot paths do not silently rot after sync.
