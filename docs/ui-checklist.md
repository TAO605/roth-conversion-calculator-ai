...
- GitHub Actions retains `health-evidence-result.json` inside `production-seo-evidence`; before launch-readiness, uptime, privacy, or incident reviews, confirm `/api/health` is 200, no-store, tax-year 2026, pending professional review, content counts, feature counts, and no secret-like keys are retained.
...
- GitHub Actions retains `search-console-verification-evidence-result.json` inside `production-seo-evidence`; before Search Console property verification retries or ownership troubleshooting, confirm `domainTxtVerified: true`, `htmlMetaVerified: true`, `spfRecordRetained: true`, `canonicalHostRetained: true`, and `gscUiOwnershipNotAsserted: true`.
...
- Run `npm run seo:gsc-query-opportunity-draft -- --query "roth conversion irmaa impact" --start 2026-06-01 --end 2026-06-05 --impressions 44 --clicks 2 --ctr 4.5 --position 9.4 --evidence docs/evidence/gsc-query.png --out docs/search-console-query-opportunity-draft.json` after a reviewer supplies a real GSC query row; confirm AI-filled cluster, intent, target surface, action, and review gate remain draft-only until reviewer evidence is accepted.
...
- Run `npm run seo:gsc-query-opportunity-backlog` before content planning; confirm local query records are summarized by status, risk, cluster, priority score, missing reviewer fields, and next action instead of becoming ad hoc keyword tasks.
